...
@Component({
selector: 'app-Complaint_Details',
templateUrl: './Complaint_Details.component.html',
styleUrls: ['./Complaint_Details.component.css'],
providers: [
    {provide: DateAdapter, useClass: MomentDateAdapter, deps: [MAT_DATE_LOCALE]},
    {provide: MAT_DATE_FORMATS, useValue: MY_FORMATS},]
})
export class Complaint_DetailsComponent implements OnInit {
    EditIndex: number;
  ...
  Complaint_Details_Data:any;
  Complaint_Details_AppliedcountdetailsData:any;
  Complaint_Details_Appliedcountdetails_Full_Data:any;
  Complaint_Details_Appliedcountdetails_Placed_Data:any;
  Complaint_Details_RejectedcountdetailsData:any;
...
Page_Load()
{
    ...
    this.Search_Complaint_Details();
}

Export()
{
    
        this.Student_Service_.exportExcel(this.Complaint_Details_Data,'Jobposting Report')
...
Export_Rejectedcount_Data()
{
        ...
        this.Student_Service_.exportExcel(this.Complaint_Details_RejectedcountdetailsData,'Jobposting Rejected Report')
...
Search_Complaint_Details()
{
   ...
   this.Total_Entries =0;
    ...
    this.Job_Posting_Service_.Search_Complaint_Details(look_In_Date_Value,moment(this.FromDate_).format('YYYY-MM-DD'), moment(this.ToDate_).format('YYYY-MM-DD'),this.Complaint_Status_,
    this.Login_User
    ,this.Pointer_Start_,
    this.Pointer_Stop_,
    this.Page_Length_).subscribe(Rows =>{
       debugger
        this.Complaint_Details_Data=Rows[0];
      
        this.Total_Entries =  this.Complaint_Details_Data.length;
        // this.Jobs =  this.Complaint_Details_Data.length;
        // this.Vacancies =  sum(0,this.Complaint_Details_Data[i].No_Of_Vaccancy);

        let Vacancies = 0;

        for (let i = 0; i < this.Complaint_Details_Data.length; i++) {
        //   Vacancies = sum(0,this.Complaint_Details_Data[i].No_Of_Vaccancy);
          Vacancies = Number(Vacancies) + Number(this.Complaint_Details_Data[i].No_Of_Vaccancy);
        }
    ...
    this.issLoading = false;
    if(this.Complaint_Details_Data.length==0)
    { 
    this.issLoading=false;
...
Next_Click() {
    ...
    if (this.Complaint_Details_Data.length == this.Page_Length_) {
  ...
  Change_Status(Complaint_Id_,Status_)
  {
    debugger
        this.issLoading = true;
        this.Student_Service_.Change_Status(this.Login_User,Complaint_Id_,Number(Status_)).subscribe(Rows => {
            if (Rows != null) {
                debugger
                this.Change_Status_Data = Rows[0];
               
            //    if(this.Change_Status_Data[0].Complaint_Id>0 && this.Change_Status_Data[0].Complaint_Complete_Status_>=0 && this.Change_Status_Data[0].Complaint_Complete_Status_!=null)
               if(this.Change_Status_Data[0].Complaint_Id_>0 )
