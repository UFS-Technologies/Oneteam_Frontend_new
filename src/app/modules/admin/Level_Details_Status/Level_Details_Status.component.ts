...
@Component({
selector: 'app-Level_Details_Status',
templateUrl: './Level_Details_Status.component.html',
styleUrls: ['./Level_Details_Status.component.css'],
providers: [
    {provide: DateAdapter, useClass: MomentDateAdapter, deps: [MAT_DATE_LOCALE]},
    {provide: MAT_DATE_FORMATS, useValue: MY_FORMATS},]
})
export class Level_Details_StatusComponent implements OnInit {
    EditIndex: number;
...
Batch_Typeahead_Service(Value,Course_Id)
{
    if (this.Batch_Data == undefined || this.Batch_Data.length==0)
    {
        ...
        this.Student_Service_.Search_Batch_Typeahead_Attendance1('',Course_Id,Number(this.Login_User)).subscribe(Rows => {
    if (Rows != null) 
    {
    ...
    Search_Level_Details_Status()
{
    ...
    this.issLoading = true;
    debugger
    this.Student_Service_.Search_Level_Details_Status(look_In_Date_Value,moment(this.FromDate_).format('YYYY-MM-DD'), moment(this.ToDate_).format('YYYY-MM-DD'),Course_Id,Batch_Id,student_name_,Faculty_Id,Number(this.Login_User)).subscribe(Rows =>{
        debugger
