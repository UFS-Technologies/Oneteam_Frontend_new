...
export class Lead_ReportComponent implements OnInit {
    ...
    Search_Enquiry_Source_=new FormControl();
    Search_Enquiry_Source_Temp:any
...
Search_Lead_Report()
{
    var  Enquiry_Source_Id = 0,look_In_Date_Value=0,Team_Member_Selection="",Enquiry_Source_Selection="",Register_Value = 1;

    // if (this.Search_Name != undefined && this.Search_Name != null && this.Search_Name != '')
    // search_name_ = this.Search_Name;
    var User_Id=0,status =0,Course_Id=0,Branch_Id_=0;
...
debugger
    this.Search_Enquiry_Source_Temp=this.Search_Enquiry_Source_;
    if (this.Search_Enquiry_Source_.value !=undefined)
    {
        for (var i=0;i<this.Search_Enquiry_Source_.value.length;i++)
        {
            Enquiry_Source_Selection=Enquiry_Source_Selection + this.Search_Enquiry_Source_.value[i].Enquiry_Source_Id.toString() +",";
        }
        if(Enquiry_Source_Selection.length>0)
        Enquiry_Source_Selection=Enquiry_Source_Selection.substring(0,Enquiry_Source_Selection.length-1)
...
Register_Value = this.Is_Registered;
    ...
    this.issLoading = true;
    this.Student_Service_.Search_Lead_Report(look_In_Date_Value,moment(this.FromDate_).format('YYYY-MM-DD'), moment(this.ToDate_).format('YYYY-MM-DD'),Enquiry_Source_Selection, this.Login_User,Team_Member_Selection,status,Course_Id,Register_Value,Branch_Id_).subscribe(Rows =>{
//debugger
    this.Lead_Report_Data = Rows[0];
debugger
    for (var i=0;i<this.Lead_Report_Data.length;i++)
    {
        if(this.Lead_Report_Data[i].Registered==1)
        {
            this.Lead_Report_Data[i].Registered_Name="Registered"
        }
        else (this.Lead_Report_Data[i].Registered_Name = "Not Registered");
