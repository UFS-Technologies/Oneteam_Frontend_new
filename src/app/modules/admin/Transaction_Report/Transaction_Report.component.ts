...
@Component({
selector: 'app-Transaction_Report',
templateUrl: './Transaction_Report.component.html',
styleUrls: ['./Transaction_Report.component.css'],
providers: [
    {provide: DateAdapter, useClass: MomentDateAdapter, deps: [MAT_DATE_LOCALE]},
    {provide: MAT_DATE_FORMATS, useValue: MY_FORMATS},]
})
export class Transaction_ReportComponent implements OnInit {
    EditIndex: number;
    ...
    issLoading: boolean;
    Permissions: any;
    Transaction_Report_Edit:boolean;
    Transaction_Report_Save:boolean;
    Transaction_Report_Delete:boolean;
    ...
    Transaction_Report_Id: number = 0;
   ...
   Search_District_Details_: State_District = new State_District;

  District_Data: State_District[];
  Districtdetails_:State_District=new State_District();
  Districtdetails_Search:State_District=new State_District();
  Districtdetails_Temp: State_District = new State_District();
  Districtdetails_Data_Filter: State_District[]   
    ...
    Login_User: number = 0;
    Transaction_Report_EditIndex: number = -1;
...
ngOnInit() 
{
    ...
    {
    this.Transaction_Report_Edit=this.Permissions.Edit;
    this.Transaction_Report_Save=this.Permissions.Save;
    this.Transaction_Report_Delete=this.Permissions.Delete;
    this.Page_Load()
...
Export()
{
        ...
        this.Student_Service_.exportExcel(this.Transaction_Report_Master_Data,'Resume Sending Report')
...
Page_Load()
{
    ...
    this.Search_Transaction_Report();
...
{
    var  Course_Id = 0, Employer_Details_Id = 0,look_In_Date_Value=0,District_Details_Id = 0;
    ...
    if (this.Course_ != undefined && this.Course_ != null)
        ...
        // if (this.Search_District_Details_ != undefined && this.Search_District_Details_ != null)
        // if (this.Search_District_Details_.District_Name != undefined && this.Search_District_Details_.State_District_Id != null)
        // District_Details_Id = this.Search_District_Details_.State_District_Id ;
        
    this.issLoading = true;
    this.Student_Service_.Search_Transaction_Report(look_In_Date_Value,moment(this.FromDate_).format('YYYY-MM-DD'), moment(this.ToDate_).format('YYYY-MM-DD'),Course_Id,Employer_Details_Id,this.Login_User).subscribe(Rows =>{
...
Search_District_Typeahead(event: any)
{     
    var Value = "";
    if (event.target.value == "")
        Value = "";
        ...
        Value = event.target.value;
    if (this.District_Data == undefined || this.District_Data.length==0)
    {
        this.issLoading = true;
        this.Student_Service_.Search_District_Typeahead('').subscribe(Rows => {
    if (Rows != null) 
    {
        this.District_Data = Rows[0];
        this.Districtdetails_Data_Filter=[];
        this.issLoading = false;

        for (var i=0;i<this.District_Data.length;i++)
        {
            if(this.District_Data[i].District_Name.toLowerCase().includes(Value))
                this.Districtdetails_Data_Filter.push(this.District_Data[i])
        }
    }
    },
    Rows => {
...
display_District(Districtdetails_e: State_District)
{     
    if (Districtdetails_e) { return Districtdetails_e.District_Name; }
}
