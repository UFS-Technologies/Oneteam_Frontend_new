...
@Component({
selector: 'app-Batch_Attendance',
templateUrl: './Batch_Attendance.component.html',
styleUrls: ['./Batch_Attendance.component.css'],
providers: [
    {provide: DateAdapter, useClass: MomentDateAdapter, deps: [MAT_DATE_LOCALE]},
    {provide: MAT_DATE_FORMATS, useValue: MY_FORMATS},]
})
export class Batch_AttendanceComponent implements OnInit {
    EditIndex: number;
    ...
    issLoading: boolean;
    Permissions: any;
    Batch_Attendance_Edit:boolean;
    Batch_Attendance_Save:boolean;
    Batch_Attendance_Delete:boolean;
    ...
    Login_User: number = 0;
    Batch_Attendance_EditIndex: number = -1;
    ...
    Present_Data:Attendance_Master[];
    Absant_Data:Attendance_Master[];

    FollowUp_Branch_: Agent = new Agent();
	Followup_Branch_Data: Agent[];
    Present_View:boolean=false;
    Absent_View:boolean=false;
    Absant_Data_count:number=0;
    Present_Data_count:number=0;


constructor(public Student_Service_:Student_Service,public Student_Data_Service_:Student_Data_Service, private route: ActivatedRoute, private router: Router,public dialogBox: MatDialog) { }
ngOnInit() 
{
    ...
    {
    this.Batch_Attendance_Edit=this.Permissions.Edit;
    this.Batch_Attendance_Save=this.Permissions.Save;
    this.Batch_Attendance_Delete=this.Permissions.Delete;
    this.Page_Load()
...
Export1()
{
        ...
        this.Student_Service_.exportExcel(this.Present_Data,'Attendance present Report')
...
Export2()
{
        ...
        this.Student_Service_.exportExcel(this.Absant_Data,'Attendance Absant Report')
...
Page_Load()
{
    ...
    this.Present_View =false;
    this.Absent_View =false;
...
{
    var  Course_Id = 0, Batch_Id = "", Faculty_Id = 0,look_In_Date_Value=0,Branch_Id_=0; 
    ...
    this.Student_Service_.Search_Batch_Attendance(moment(this.FromDate_).format('YYYY-MM-DD'), moment(this.ToDate_).format('YYYY-MM-DD'),Faculty_Id,Course_Id,Batch_Id,Attendance_Status_Id,this.Login_User,look_In_Date_Value,Branch_Id_).subscribe(Rows =>{
       debugger 
...
Search_Branch_Typeahead(event: any) {
    ...
    present_count_Details(Attendance_Master_,index)
    {
        debugger
       
        this.Present_View =true;
        this.Absent_View =false;
        this.Entry_View=true;

        this.issLoading = true;
        
        this.Student_Service_.Load_Batch_Attendance_Details(Attendance_Master_.Student_Id,Attendance_Master_.Batch_Id,Attendance_Master_.Course_Id,1).subscribe(Rows =>{
           debugger 
        this.Present_Data = Rows[0];
        this.Present_Data_count = this.Present_Data.length
        
        this.issLoading = false;
        if(this.Present_Data.length==0)
        { 
        this.issLoading=false;
        const dialogRef = this.dialogBox.open( DialogBox_Component, {panelClass:'Dialogbox-Class',data:{Message:'No Details Found',Type:"3"}});
        }
    ...
    absent_count_Details(Attendance_Master_,index)
    {
        this.Present_View =false;
        this.Absent_View =true;
        this.Entry_View=true;

        this.issLoading = true;
        
        this.Student_Service_.Load_Batch_Attendance_Details(Attendance_Master_.Student_Id,Attendance_Master_.Batch_Id,Attendance_Master_.Course_Id,0).subscribe(Rows =>{
           debugger 
        this.Absant_Data = Rows[0];
        this.Absant_Data_count = this.Absant_Data.length
        this.issLoading = false;
        if(this.Absant_Data.length==0)
        { 
        this.issLoading=false;
        const dialogRef = this.dialogBox.open( DialogBox_Component, {panelClass:'Dialogbox-Class',data:{Message:'No Details Found',Type:"3"}});
        }
