...
@Component({
    selector: 'app-Syllabus_Coverage',
    templateUrl: './Syllabus_Coverage.component.html',
    styleUrls: ['./Syllabus_Coverage.component.css'],
    providers: [
        {provide: DateAdapter, useClass: MomentDateAdapter, deps: [MAT_DATE_LOCALE]},
        {provide: MAT_DATE_FORMATS, useValue: MY_FORMATS},]
    })
export class Syllabus_CoverageComponent implements OnInit {
    EditIndex: number;
    ...
    issLoading: boolean;
    Permissions: any;
    Syllabus_Coverage_Edit:boolean;
    Syllabus_Coverage_Save:boolean;
    Syllabus_Coverage_Delete:boolean;
    ...
    Login_User: number = 0;
    Syllabus_Coverage_EditIndex: number = -1;
...
ngOnInit() 
{
    ...
    this.Permissions = Get_Page_Permission(79);
    ...
    {
    this.Syllabus_Coverage_Edit=this.Permissions.Edit;
    this.Syllabus_Coverage_Save=this.Permissions.Save;
    this.Syllabus_Coverage_Delete=this.Permissions.Delete;
    this.Page_Load()
...
Export()
{
        ...
        this.Student_Service_.exportExcel(this.Syllabus_Coverage_Data,'Syllabus Coverage Report')
...
Edit_Lead(Student_Id, i) {
    ...
    this.Edit_Page_Permission = Get_Page_Permission(79);
...
Search_Syllabus_Coverage()
{
    ...
    debugger
    this.issLoading = true;
    this.Student_Service_.Search_Syllabus_Coverage(look_In_Date_Value,moment(this.FromDate_).format('YYYY-MM-DD'), moment(this.ToDate_).format('YYYY-MM-DD'),Batch_Id,Faculty_Id,Course_Id,this.Login_User,FollowUp_Branch_Id).subscribe(Rows =>{
debugger
    this.Syllabus_Coverage_Data = Rows[0];
debugger
    // for (var i=0;i<this.Syllabus_Coverage_Data.length;i++)
    // {
    //     if(this.Syllabus_Coverage_Data[i].conversion_count<=25)
    //     {
    //         this.Syllabus_Coverage_Data[i].convesion_color=1
    //     }
    ...
    // for (var i=0;i<this.Syllabus_Coverage_Data.length;i++)
    // {
    //     if(this.Syllabus_Coverage_Data[i].conversion_count>=25 && this.Syllabus_Coverage_Data[i].conversion_count<=45)
    //     {
    //         this.Syllabus_Coverage_Data[i].convesion_color=2
    //     }
    ...
    // for (var i=0;i<this.Syllabus_Coverage_Data.length;i++)
    // {
    //     if(this.Syllabus_Coverage_Data[i].conversion_count>=46)
    //     {
    //         this.Syllabus_Coverage_Data[i].convesion_color=3
    //     }
    ...
    this.Total_Entries = this.Syllabus_Coverage_Data.length;
    this.issLoading = false;
    if(this.Syllabus_Coverage_Data.length==0)
    { 
    this.issLoading=false;
...
Mark_As_Complete(Batch_Id_,Status_)
{
    ...
    this.Student_Service_.Mark_As_Complete(this.Login_User,Batch_Id_,Number(Status_)).subscribe(Rows => {
        if (Rows != null) {
           ...
           if(this.Mark_As_Complete_Data[0].Batch_Id_>0 && this.Mark_As_Complete_Data[0].Batch_Complete_Status_>0 )
           {
