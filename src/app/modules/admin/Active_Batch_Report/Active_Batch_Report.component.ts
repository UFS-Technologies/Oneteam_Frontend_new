...
@Component({
selector: 'app-Active_Batch_Report',
templateUrl: './Active_Batch_Report.component.html',
styleUrls: ['./Active_Batch_Report.component.css'],
providers: [
    {provide: DateAdapter, useClass: MomentDateAdapter, deps: [MAT_DATE_LOCALE]},
    {provide: MAT_DATE_FORMATS, useValue: MY_FORMATS},]
})
export class Active_Batch_ReportComponent implements OnInit {
    EditIndex: number;
    ...
    issLoading: boolean;
    Permissions: any;
    Active_Batch_Report_Edit:boolean;
    Active_Batch_Report_Save:boolean;
    Active_Batch_Report_Delete:boolean;
    ...
    Login_User: number = 0;
    Active_Batch_Report_EditIndex: number = -1;
...
ngOnInit() 
{
    ...
    this.Permissions = Get_Page_Permission(75);
    ...
    {
    this.Active_Batch_Report_Edit=this.Permissions.Edit;
    this.Active_Batch_Report_Save=this.Permissions.Save;
    this.Active_Batch_Report_Delete=this.Permissions.Delete;
    this.Page_Load()
...
Page_Load()
{
    ...
    this.Search_Active_Batch_Report();
}

Export()
{
    
        this.Student_Service_.exportExcel(this.Active_Batch_Report_Data,'Upcoming Batch Data')
...
Edit_Lead(Student_Id, i) {
    ...
    this.Edit_Page_Permission = Get_Page_Permission(75);
...
Search_Active_Batch_Report()
{
    ...
    debugger
    this.issLoading = true;
    this.Student_Service_.Search_Active_Batch_Report(look_In_Date_Value,moment(this.FromDate_).format('YYYY-MM-DD'), moment(this.ToDate_).format('YYYY-MM-DD'),Batch_Id,Faculty_Id,Course_Id,this.Login_User,FollowUp_Branch_Id).subscribe(Rows =>{
debugger
    this.Active_Batch_Report_Data = Rows[0];
debugger
    // for (var i=0;i<this.Active_Batch_Report_Data.length;i++)
    // {
    //     if(this.Active_Batch_Report_Data[i].conversion_count<=25)
    //     {
    //         this.Active_Batch_Report_Data[i].convesion_color=1
    //     }
    ...
    // for (var i=0;i<this.Active_Batch_Report_Data.length;i++)
    // {
    //     if(this.Active_Batch_Report_Data[i].conversion_count>=25 && this.Active_Batch_Report_Data[i].conversion_count<=45)
    //     {
    //         this.Active_Batch_Report_Data[i].convesion_color=2
    //     }
    ...
    // for (var i=0;i<this.Active_Batch_Report_Data.length;i++)
    // {
    //     if(this.Active_Batch_Report_Data[i].conversion_count>=46)
    //     {
    //         this.Active_Batch_Report_Data[i].convesion_color=3
    //     }
    ...
    this.Total_Entries = this.Active_Batch_Report_Data.length;

    for(var i=0;i<this.Active_Batch_Report_Data.length;i++)
				{
					this.Total_Student =
						Number(this.Total_Student) + Number(this.Active_Batch_Report_Data[i].Current_Admission_Count);
				 
		}

    this.issLoading = false;
    if(this.Active_Batch_Report_Data.length==0)
    { 
    this.issLoading=false;
